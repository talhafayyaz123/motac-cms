const generateDummyData = () => {
  ...
  const prices = ['MYR 250', 'MYR 50', 'MYR 30', 'MYR 20'];

  const tags = [
    ['Scenery', 'Nature', 'Oil Painting'],
    ['Scenery', 'Nature', 'Oil Painting'],
  ];
  ...
  const data = Array.from({ length: 18 }, (_, index) => {
    return {
      ...
      'Product ID': `ART00${(index % 5) + 1}`,
      'Product Name': products[index % products.length],
      'Product Category': categories[index % categories.length],
      'Product Price': prices[index % prices.length],
      Tags: tags[index % tags.length],
      'Product Upload Date': `2023-04-${15 - (index % 5)}`,
